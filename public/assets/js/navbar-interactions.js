 ...
 * - Handles global search debounced input
 * - Loads and renders notifications, exposes helpers for dismissal
 */
(function () {
  'use strict';
  ...
  function initNotifications() {
    ...
    notificationDropdown = document.querySelector('.notification-menu');
  ...
  function loadNotifications() {
    return fetch('/api/get-notifications')
      ...
      .catch((error) => {
        warn('Error loading notifications:', error);
  ...
  function displaySummary(summary) {
    ...
    const unreadTotal = typeof summary.unread_total === 'number' ? summary.unread_total : 0;
    const highPriorityTotal = typeof summary.high_priority_total === 'number' ? summary.high_priority_total : 0;
    const lastUpdated = summary.last_updated || 'just now';
  ...
  function updateBadge(count) {
    ...
    if (count > 0) {
      badgeElement.textContent = count > 9 ? '9+' : String(count);
    ...
    fetch('/api/notifications?action=dismiss', {
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          ...
          const currentCount = badgeElement ? parseInt(badgeElement.textContent, 10) : 0;
          if (badgeElement && !Number.isNaN(currentCount)) {
            const newCount = Math.max(0, currentCount - 1);
            badgeElement.textContent = newCount > 9 ? '9+' : String(newCount);
            if (newCount === 0) {
              badgeElement.style.display = 'none';
            }
          }
      ...
      .catch((error) => {
        warn('Error dismissing notification:', error);
        if (typeof showToast === 'function') {
          showToast('Failed to dismiss notification', 'error');
        }
      });
