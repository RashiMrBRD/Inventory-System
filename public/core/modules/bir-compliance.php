...
foreach ($recentForms as $f) {
    if (!empty($f["due_date"])) {
        $ts = is_string($f["due_date"])
            ? strtotime($f["due_date"])
            : (is_object($f["due_date"]) &&
            method_exists($f["due_date"], "toDateTime")
                ? $f["due_date"]->toDateTime()->getTimestamp()
                : null);
        if (
            $ts &&
            $ts >= strtotime("today") &&
            ($nextFiling === null || $ts < $nextFiling)
        ) {
            $nextFiling = $ts;
        }
    }
    ...
    <div class="page-banner-left">
      ...
      <div class="page-banner-meta">
        ...
        <div class="page-banner-meta-item">
          ...
          <span class="font-medium"><?php echo CurrencyHelper::symbol() .
              " " .
              CurrencyHelper::getCurrentCurrency(); ?></span>
        </div>
        <?php if ($birAlerts > 0): ?>
        <div class="page-banner-meta-item">
          <strong>Active Alerts:</strong>
          <span class="badge badge-warning"><?php echo number_format(
              $birAlerts,
          ); ?></span>
        </div>
        <?php endif; ?>
        ...
        <div class="page-banner-meta-item">
          ...
          <span class="text-warning"><?php echo date(
              "F j, Y",
              $nextFiling,
          ); ?></span>
        </div>
...
<div class="section" style="margin-top: 3rem;">
  <div class="section-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
    ...
    <div class="flex items-center gap-3" style="flex-wrap: wrap;">
      <span class="text-sm text-muted-foreground" style="color: #64748b;"><?php echo count(
          $recentForms,
      ); ?> total forms</span>
        ...
        <tr style="border-bottom: 1px solid #f1f5f9; transition: all 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'; this.style.borderColor='#e2e8f0';" onmouseout="this.style.backgroundColor='transparent'; this.style.borderColor='#f1f5f9';">
          <td style="padding: 1rem 1.5rem;">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
              ...
              <div>
                <div style="font-weight: 600; color: #0f172a; font-size: 0.875rem;"><?php echo htmlspecialchars(
                    $form["form_type"],
                ); ?></div>
                <div style="font-size: 0.75rem; color: #64748b; margin-top: 0.125rem;">ID: <?php echo substr(
                    $form["_id"],
                    -8,
                ); ?></div>
              </div>
          ...
          <td style="padding: 1rem 1.5rem;">
            ...
            if ($period) {
                ...
                echo '<div style="font-weight: 500; color: #0f172a;">' .
                    $date->format("M Y") .
                    "</div>";
                echo '<div style="font-size: 0.75rem; color: #64748b;">' .
                    $date->format("F j, Y") .
                    "</div>";
            } else {
          ...
          <td style="padding: 1rem 1.5rem;">
            <div style="font-weight: 600; color: #0f172a; font-size: 0.875rem;"><?php echo CurrencyHelper::format(
                $form["total_duties"] ?? 0,
            ); ?></div>
            <?php if (
                !empty($form["customs_duty"]) ||
                !empty($form["import_vat"])
            ): ?>
            <div style="font-size: 0.75rem; color: #64748b; margin-top: 0.125rem;">
              <?php if (!empty($form["customs_duty"])) {
                  echo "Duty: " . CurrencyHelper::format($form["customs_duty"]);
              } ?>
              <?php if (
                  !empty($form["customs_duty"]) &&
                  !empty($form["import_vat"])
              ...
              <?php if (!empty($form["import_vat"])) {
                  echo "VAT: " . CurrencyHelper::format($form["import_vat"]);
              } ?>
            </div>
            <?php endif; ?>
          </td>
          <td style="padding: 1rem 1.5rem;">
            ...
            <div class="status-buttons" style="display: flex; gap: 0.25rem; flex-wrap: wrap;">
              <?php foreach ($statusConfig as $value => $status): ?>
                <button
                  onclick="updateFormStatus('<?php echo $form[
                      "_id"
                  ]; ?>', '<?php echo $value; ?>')"
                  style="
                    background: <?php echo $currentStatus === $value
                        ? $status["bg"]
                        : "#f8fafc"; ?>;
                    color: <?php echo $currentStatus === $value
                        ? $status["color"]
                        : "#64748b"; ?>;
                    border: 1px solid <?php echo $currentStatus === $value
                        ? $status["border"]
                        : "#e2e8f0"; ?>;
                    ...
                    white-space: nowrap;
                  "
                  onmouseover="this.style.background='<?php echo $currentStatus ===
                  $value
                      ? $status["bg"]
                      : "#f1f5f9"; ?>'; this.style.borderColor='<?php echo $status[
    "border"
]; ?>';"
                  onmouseout="this.style.background='<?php echo $currentStatus ===
                  $value
                      ? $status["bg"]
                      : "#f8fafc"; ?>'; this.style.borderColor='<?php echo $currentStatus ===
$value
    ? $status["border"]
    : "#e2e8f0"; ?>';"
                >
          ...
          <td style="padding: 1rem 1.5rem;">
            <div style="display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
              <button onclick="viewForm('<?php echo $form[
                  "_id"
              ]; ?>')" style="padding: 0.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0; background: white; color: #475569; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center;" onmouseover="this.style.background='#f8fafc'; this.style.borderColor='#cbd5e1';" onmouseout="this.style.background='white'; this.style.borderColor='#e2e8f0';" title="View Form">
              ...
              <button onclick="deleteForm('<?php echo $form[
                  "_id"
              ]; ?>')" style="padding: 0.5rem; border-radius: 0.5rem; border: 1px solid #fecaca; background: white; color: #dc2626; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center;" onmouseover="this.style.background='#fef2f2'; this.style.borderColor='#fca5a5';" onmouseout="this.style.background='white'; this.style.borderColor='#fecaca';" title="Delete Form">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="form-card" style="background: white; border-radius: 0.75rem; border: 1px solid #e2e8f0; margin-bottom: 1rem; overflow: hidden; transition: all 0.2s;" onmouseover="this.style.boxShadow='0 4px 6px -1px rgb(0 0 0 / 0.1)';" onmouseout="this.style.boxShadow='0 1px 3px 0 rgb(0 0 0 / 0.1)';">
      ...
      <div style="padding: 1rem; border-bottom: 1px solid #f1f5f9; background: linear-gradient(to right, #f8fafc, #f1f5f9);">
        <div style="display: flex; justify-content: space-between; align-items: center; gap: 1rem;">
          <div style="display: flex; align-items: center; gap: 0.75rem;">
            <div style="width: 2px; height: 1.5rem; background: linear-gradient(to bottom, #000000ff, #620679ff); border-radius: 1px;"></div>
            <div>
              <div style="font-weight: 600; color: #0f172a; font-size: 0.875rem;"><?php echo htmlspecialchars(
                  $form["form_type"],
              ); ?></div>
              <div style="font-size: 0.75rem; color: #64748b;">ID: <?php echo substr(
                  $form["_id"],
                  -8,
              ); ?></div>
            </div>
          ...
          <div style="text-align: right;">
            <div style="font-weight: 600; color: #0f172a; font-size: 1rem;"><?php echo CurrencyHelper::format(
                $form["total_duties"] ?? 0,
            ); ?></div>
            <?php if (
                !empty($form["customs_duty"]) ||
...
$value
    ...
    : "#e2e8f0"; ?>';"
        ...
        <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
          ...
          <button onclick="deleteForm('<?php echo $form[
              "_id"
          ]; ?>')" style="padding: 0.5rem 1rem; border-radius: 0.5rem; border: 1px solid #fecaca; background: white; color: #dc2626; cursor: pointer; transition: all 0.2s; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; gap: 0.5rem;" onmouseover="this.style.background='#fef2f2'; this.style.borderColor='#fca5a5';" onmouseout="this.style.background='white'; this.style.borderColor='#fecaca';">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M3 6H5H21M8 6V4C8 3.44772 8.44772 3 9 3H15C15.5523 3 16 3.44772 16 4V6M19 6V20C19 21.1046 18.1046 22 17 22H7C5.89543 22 5 21.1046 5 20V6H19Z"/>
              <path d="M10 11V17M14 11V17"/>
            </svg>
...
<div id="itrModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 1000; overflow: auto;">
  <div style="position: relative; background: white; margin: 1rem auto; padding: 0; width: 95%; max-width: 1400px; max-height: 95vh; overflow-y: auto; border-radius: 0.25rem; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.15);">
    ...
    <div style="background: #f8f9fa; padding: 1.5rem; border-bottom: 2px solid #000; text-align: center;">
      ...
      <div style="font-size: 0.75rem; color: #666; margin-top: 0.5rem;">Tax Year: <span id="taxYear"><?php echo date(
          "Y",
      ); ?></span></div>
    </div>
      ...
      <div style="margin-bottom: 2rem;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
          <div>
            ...
            <input type="text" id="tin" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
          </div>
          <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">2. RDO Code</label>
            <input type="text" id="rdoCode" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
          </div>
        </div>

        <div style="margin-bottom: 1rem;">
          <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">3. Registered Name</label>
          <input type="text" id="registeredName" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
        </div>

        <div style="margin-bottom: 1rem;">
          <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">4. Registered Address</label>
          <input type="text" id="registeredAddress" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
          <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">5. Zip Code</label>
            <input type="text" id="zipCode" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
          </div>
          <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">6. Telephone Number</label>
            <input type="text" id="telephone" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
          </div>
          <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; margin-bottom: 0.25rem;">7. Email Address</label>
            <input type="email" id="email" style="width: 100%; padding: 0.5rem; border: 1px solid #ccc; font-size: 0.875rem;" placeholder="">
          </div>
        </div>
...
async function generateRAMSAY307() {
  try {
    ...
    // Try to get recent container data
    try {
      ...
      if (data.success && data.data && data.data.length > 0) {
        container = data.data[0]; // Use most recent container
        console.log('Using container data:', container);
      }
    } catch (shipmentError) {
      console.log('Could not fetch shipment data, using sample data:', shipmentError);
    }

    // Use sample data if no shipment data available
    if (!container) {
      const valueOfGoods = parseFloat(prompt('Enter value of goods (PHP):', '50000')) || 50000;
      container = {
        container_number: '',
        arrival_date: new Date().toISOString().split('T')[0],
        value_of_goods: valueOfGoods,
        shipping_cost: valueOfGoods * 0.05,
        insurance_amount: valueOfGoods * 0.01,
        origin_country: 'China',
        description: 'Imported Goods'
      };
      console.log('Using sample container data:', container);
    }
        ...
        <div class="form-section" style="background: #f8fafc; border-radius: 0.5rem; padding: 1.25rem; border: 1px solid #e2e8f0; height: fit-content;">
          ...
          <div style="display: flex; flex-direction: column; gap: 0.875rem;">
            <div>
              ...
              <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">${formData['form type'] || 'N/A'}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">${formData['arrival date'] || 'N/A'}</div>
            </div>
            <div>
              ...
              <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">${formData['created'] || 'N/A'}</div>
            </div>
          </div>
        </div>
        ...
        <div class="form-section" style="background: #f8fafc; border-radius: 0.5rem; padding: 1.25rem; border: 1px solid #e2e8f0; height: fit-content;">
          ...
          <div style="display: flex; flex-direction: column; gap: 1rem;">
            <div>
              ...
              <div style="font-weight: 600; color: #0f172a; font-size: 1.125rem;">${CurrencyHelper.format(formData['value of goods'] || 0)}</div>
            </div>
            <div>
              ...
              <div style="font-weight: 600; color: #0f172a; font-size: 1.125rem;">${CurrencyHelper.format(formData['customs duty'] || 0)}</div>
            </div>
            <div>
              ...
              <div style="font-weight: 600; color: #0f172a; font-size: 1.125rem;">${CurrencyHelper.format(formData['import vat'] || 0)}</div>
            </div>
            <div>
              ...
              <div style="font-weight: 600; color: #0f172a; font-size: 1.125rem;">${CurrencyHelper.format(formData['other taxes'] || 0)}</div>
            </div>
          </div>
        </div>
      </div>
...
function createTextFile(formData, title, subtitle, formId) {
  ...
  lines.push(`Form Type: ${formData['form type'] || 'N/A'}`);
  lines.push(`Container Number: ${formData['container number'] || 'N/A'}`);
  lines.push(`Arrival Date: ${formData['arrival date'] || 'N/A'}`);
  lines.push(`Created: ${formData['created'] || 'N/A'}`);
  lines.push('');

  lines.push('FINANCIAL INFORMATION');
  lines.push('-'.repeat(40));
  lines.push(`Value of Goods: ${formData['value of goods'] || 'N/A'}`);
  lines.push(`Customs Duty: ${formData['customs duty'] || 'N/A'}`);
  lines.push(`Import VAT: ${formData['import vat'] || 'N/A'}`);
  lines.push(`Other Taxes: ${formData['other taxes'] || 'N/A'}`);
  lines.push('');

  lines.push('='.repeat(80));
  lines.push('TOTAL DUTIES AND TAXES');
  lines.push(formData['total_duties'] || 'N/A');
  lines.push('');
  lines.push('GRAND TOTAL (Value of Goods + Duties)');

  // Calculate grand total
  const valueOfGoods = parseFloat((formData['value of goods'] || '₱0').replace(/[₱,]/g, ''));
  const totalDuties = parseFloat((formData['total_duties'] || '₱0').replace(/[₱,]/g, ''));
  const grandTotal = valueOfGoods + totalDuties;
  lines.push(`₱${grandTotal.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
  lines.push('');
  lines.push('Final Amount Due (Including Value of Goods)');
  lines.push('='.repeat(80));
...
function downloadFormFromModal() {
  ...
  // Create text file
  createTextFile(formData, modalTitle, modalSubtitle, formId);
}
...
function generateITR() {
  Toast.loading('Loading Annual ITR Form...');
  ...
  fetch('../../api/financial-statements.php?action=get_annual_data')
    ...
    .then(result => {
      ...
      if (result.success) {
        ...
        Toast.success('ITR Form loaded successfully');
      } else {
        // Still show the modal with default data
        renderITRInModal(null);
        Toast.warning('Using sample data - actual financial data not available');
      }
    })
    .catch(error => {
      console.error('Error fetching financial data:', error);
      Toast.clearAll();
      // Still show the modal with default data on error
      renderITRInModal(null);
      Toast.warning('Using sample data - could not fetch financial data');
    });
}
...
function populateITRForm(data) {
  // Populate taxpayer information
  if (data && data.company_info) {
    const ci = data.company_info;
    document.getElementById('tin').value = ci.tin || '000-000-000-000';
    document.getElementById('rdoCode').value = ci.rdo_code || '128';
    document.getElementById('registeredName').value = ci.name || 'SAMPLE CORPORATION';
    document.getElementById('registeredAddress').value = ci.address || '123 Sample Street, Makati City, Philippines';
    document.getElementById('zipCode').value = ci.zip_code || '1200';
    document.getElementById('telephone').value = ci.telephone || '(02) 8888-8888';
    document.getElementById('email').value = ci.email || 'sample@corporation.com';
  } else {
    // Default taxpayer info
  ...
  if (data && data.financial_statements) {
    ...
    document.getElementById('grossSales').value = fs.revenue || fs.gross_income || 0;
    ...
    document.getElementById('taxCredits').value = fs.tax_withheld || fs.tax_credits || 0;
  } else {
  ...
  let html = `
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
      ...
      <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 1.25rem; font-weight: 600; color: #0f172a;">Company Information</h3>
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <div>
      ...
      <!-- Tax Summary -->
      <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 1.25rem; font-weight: 600; color: #0f172a;">Tax Summary</h3>
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${grossIncome.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${allowableDeductions.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${taxableIncome.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
        </div>
      </div>
      ...
      <!-- Tax Computation -->
      <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 1.25rem; font-weight: 600; color: #0f172a;">Tax Computation</h3>
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${regularTax.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${mcit.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            ...
            <div style="font-weight: 600; color: #16a34a; font-size: 0.875rem;">₱${incomeTax.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            <div style="font-size: 0.75rem; color: #64748b; margin-bottom: 0.25rem; text-transform: uppercase; letter-spacing: 0.05em;">Other Taxes</div>
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${otherTaxes.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
        </div>
      </div>
      ...
      <!-- Total Tax Due -->
      <div style="background: #f8fafc; padding: 1.5rem; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 1.25rem; font-weight: 600; color: #0f172a;">Total Tax Due</h3>
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <div>
            ...
            <div style="font-weight: 500; color: #0f172a; font-size: 0.875rem;">₱${incomeTax.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
    ...
    <div style="margin-top: 2rem; padding: 2rem; background: #0f172a; border: 2px solid #334155; border-radius: 0.5rem; text-align: center;">
      ...
      <div style="font-size: 2rem; font-weight: 700; color: white; margin-bottom: 0.5rem;">₱${totalTax.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
...
function calculateITR() {
  // Helper: parse numbers safely (handles commas)
  ...
  // Helper: format as plain number string (inputs are type=number in some places, but we keep it consistent)
  ...
  const grossSales = parseNumber(document.getElementById('grossSales').value);
  const costOfGoods = parseNumber(document.getElementById('costOfGoods').value);
  ...
  const dividendIncome = parseNumber(document.getElementById('dividendIncome').value);
  const taxCredits = parseNumber(document.getElementById('taxCredits').value);

  // PART III: Net Taxable Income (Gross Income - Deductions)
  ...
  // PART IV: COMPUTATION OF TAX (CREATE Law rates)
  // 41. Normal Income Tax Due = 25% of Item 40
  const normalTaxDue = netTaxableIncome * 0.25;

  // 42. MCIT = 1% of Gross Sales/Receipts (Item 17A)
  const mcitDue = grossSales * 0.01;

  // 43. Tax Due = higher of Item 41 or 42
  const taxDue = Math.max(normalTaxDue, mcitDue);

  // PART II: Net Tax Payable = Tax Due - Credits (not below 0)
  const netTaxPayable = Math.max(0, taxDue - taxCredits);

  // Update PART III fields
  document.getElementById('netTaxableIncome').value = formatNumber(netTaxableIncome);

  // Update PART IV fields (these were becoming “empty” because the code was writing locale-formatted strings into number inputs)
  document.getElementById('netTaxableIncome2').value = formatNumber(netTaxableIncome);
  document.getElementById('normalTaxDue').value = formatNumber(normalTaxDue);
  document.getElementById('mcitDue').value = formatNumber(mcitDue);
  document.getElementById('taxDue').value = formatNumber(taxDue);

  // Update PART II fields
  document.getElementById('totalTaxDue').value = formatNumber(taxDue);
  document.getElementById('netTaxPayable').value = formatNumber(netTaxPayable);

  // Update status badge
  const statusElement = document.querySelector('#itrStatus span:last-child');
  const taxTypeApplied = mcitDue > normalTaxDue ? 'MCIT APPLIED' : 'CALCULATED';
  statusElement.textContent = taxTypeApplied;
  statusElement.style.background = '#28a745';
  statusElement.style.color = 'white';

  // Show notification
  Toast.success(`Tax calculated: ₱${taxDue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} due`);
  setTimeout(() => Toast.clearAll(), 3000);
}
