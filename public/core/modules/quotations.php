...
if (!$user || !is_array($user)) {
    error_log('Quotations: User data is null or invalid');
...
<div class="toolbar">
  ...
  <div class="toolbar-right">
    ...
    <button class="btn btn-ghost btn-icon" data-action="print-quotes" title="Print Quotations">
    ...
    <button class="btn btn-ghost btn-icon" data-action="export-quotes" title="Export to CSV/Excel">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
    ...
    <button class="btn btn-ghost btn-icon" data-action="bulk-actions-menu" title="Bulk Actions">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
  ...
  <table class="data-table">
    <thead>
      <tr>
        <th class="checkbox-column" style="display: none;">
          <input type="checkbox" id="selectAll" data-action="toggle-select-all" style="cursor: pointer;">
        </th>
        <th class="<?php echo !empty($quotations) ? 'sortable-header' : ''; ?>" data-action="sort-column" <?php echo !empty($quotations) ? 'data-sort-column="quote_number" style="cursor: pointer; user-select: none;"' : ''; ?>>
        ...
        <th class="<?php echo !empty($quotations) ? 'sortable-header' : ''; ?>" data-action="sort-column" <?php echo !empty($quotations) ? 'data-sort-column="customer" style="cursor: pointer; user-select: none;"' : ''; ?>>
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            <span>Customer</span>
            <?php if (!empty($quotations)): ?>
        ...
        <th class="<?php echo !empty($quotations) ? 'sortable-header' : ''; ?>" data-action="sort-column" <?php echo !empty($quotations) ? 'data-sort-column="date" style="cursor: pointer; user-select: none;"' : ''; ?>>
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            <span>Date</span>
            <?php if (!empty($quotations)): ?>
        ...
        <th class="<?php echo !empty($quotations) ? 'sortable-header' : ''; ?>" data-action="sort-column" <?php echo !empty($quotations) ? 'data-sort-column="amount" style="cursor: pointer; user-select: none;"' : ''; ?>>
          <div style="display: flex; align-items: center; gap: 0.5rem;">
        ...
        <th class="<?php echo !empty($quotations) ? 'sortable-header' : ''; ?>" data-action="sort-column" <?php echo !empty($quotations) ? 'data-sort-column="status" style="cursor: pointer; user-select: none;"' : ''; ?>>
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            <span>Status</span>
            <?php if (!empty($quotations)): ?>
    ...
    <tbody>
      ...
      <tr data-quote-id="<?php echo $quote['id']; ?>" data-status="<?php echo $quote['status']; ?>">
        <th class="checkbox-column" style="display: none;">
          <input type="checkbox" class="quote-checkbox" value="<?php echo $quote['id']; ?>" style="cursor: pointer;">
        </th>
        ...
        <td>
          <div class="flex gap-1">
            <button class="btn btn-ghost btn-sm" data-action="view-quote" data-quote-id="<?php echo $quote['id']; ?>" title="View Details">
            ...
            <button class="btn btn-ghost btn-sm text-success" data-action="approve-quote" data-quote-id="<?php echo $quote['id']; ?>" title="Approve">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            ...
            <button class="btn btn-ghost btn-sm text-primary" data-action="convert-to-order" data-quote-id="<?php echo $quote['id']; ?>" title="Convert to Order">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            ...
            <?php endif; ?>
            <button class="btn btn-ghost btn-sm" data-action="show-quote-actions" data-quote-id="<?php echo $quote['id']; ?>" data-status="<?php echo $quote['status']; ?>" title="More Actions">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
...
<div id="emptyStateContainer" style="display: <?php echo empty($quotations) ? 'block' : 'none'; ?>; padding: 4rem 2rem; text-align: center; background: white; border-radius: 8px; border: 1px solid #e5e7eb;">
  ...
  <button data-action="show-new-quote" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 0.5rem;">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5V19M5 12H19" stroke-linecap="round"/></svg>
...
<div id="paginationControls" style="display: <?php echo (count($quotations) === 0 || count($quotations) <= 6) ? 'none' : 'flex'; ?>; justify-content: space-between; align-items: center; margin-top: 1.5rem; padding: 1rem; background: white; border-radius: 8px; border: 1px solid #e5e7eb;">
  ...
  <div style="display: flex; gap: 0.5rem;">
    <button id="prevPage" data-action="change-page" data-direction="-1" class="btn btn-ghost btn-sm" style="padding: 0.5rem 1rem;" disabled>
    ...
    <button id="nextPage" data-action="change-page" data-direction="1" class="btn btn-ghost btn-sm" style="padding: 0.5rem 1rem;">
      Next
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  <div style="display: flex; align-items: center; gap: 0.5rem;">
    ...
    <select id="itemsPerPage" data-action="change-items-per-page" style="padding: 0.375rem 0.75rem; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.875rem; cursor: pointer;">
...
<div id="viewQuoteModal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(8px); z-index: 9999; align-items: center; justify-content: center; padding: 2rem;">
  <div style="width: 100%; max-width: 900px; max-height: 90vh; background: white; border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.35); display: flex; flex-direction: column; overflow: hidden;">
    ...
    <div style="background: hsl(210 20% 98%); padding: 1rem 2rem; border-top: 1px solid hsl(214 20% 88%); display: flex; gap: 0.75rem; justify-content: flex-end;">
      <button type="button" data-action="download-quote-pdf" class="btn btn-ghost">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>
        Download PDF
      </button>
      <button type="button" data-action="close-view-modal" class="btn btn-primary">Close</button>
    </div>
  </div>
</div>
...
<div id="actionsDropdown" style="display: none; position: absolute; background: white; border: 1px solid hsl(214 20% 88%); border-radius: 8px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.15); z-index: 10000; min-width: 200px;">
  <div style="padding: 0.5rem;">
    ...
    <button data-action="quote-dropdown" data-quote-action="download" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(222 47% 17%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(210 20% 98%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>
      Download PDF
    </button>
    <button data-action="quote-dropdown" data-quote-action="duplicate" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(222 47% 17%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(210 20% 98%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
      Duplicate
    </button>
    <div style="height: 1px; background: hsl(214 20% 88%); margin: 0.5rem 0;"></div>
    <button id="actionVoid" data-action="quote-dropdown" data-quote-action="void" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(25 95% 45%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(48 96% 95%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
      Void Quote
    </button>
    <button data-action="quote-dropdown" data-quote-action="delete" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(0 74% 50%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(0 86% 97%)'" onmouseout="this.style.background='transparent'">
...
<div id="bulkActionsDropdown" style="display: none; position: fixed; background: white; border: 1px solid hsl(214 20% 88%); border-radius: 8px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.15); z-index: 10000; min-width: 220px;">
  ...
  <div style="padding: 0.5rem;">
    <button data-action="bulk-dropdown" data-bulk-action="approve" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(140 61% 35%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(143 85% 96%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 13l4 4L19 7"/></svg>
      Approve Selected
    </button>
    <button data-action="bulk-dropdown" data-bulk-action="void" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(25 95% 45%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(48 96% 95%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
      Void Selected
    </button>
    <button data-action="bulk-dropdown" data-bulk-action="email" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(222 47% 17%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(210 20% 98%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
      Email Selected
    </button>
    <button data-action="bulk-dropdown" data-bulk-action="export" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(222 47% 17%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(210 20% 98%)'" onmouseout="this.style.background='transparent'">
    ...
    <div style="height: 1px; background: hsl(214 20% 88%); margin: 0.5rem 0;"></div>
    <button data-action="bulk-dropdown" data-bulk-action="delete" style="width: 100%; display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.875rem; border: none; background: transparent; border-radius: 6px; cursor: pointer; font-size: 0.875rem; color: hsl(0 74% 50%); text-align: left; transition: background 0.2s;" onmouseover="this.style.background='hsl(0 86% 97%)'" onmouseout="this.style.background='transparent'">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
      Delete Selected
    </button>
  </div>
...
<div id="newQuoteModal" data-action="attempt-close-quote-modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 50; align-items: center; justify-content: center; padding: 2rem; backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px);">
  <div onclick="event.stopPropagation()" style="width: 100%; max-width: 1200px; max-height: 92vh; background: white; border-radius: 20px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.35), 0 10px 25px -5px rgba(0,0,0,0.15); animation: slideUp 0.4s cubic-bezier(0.4, 0, 0.2, 1); display: flex; flex-direction: column; overflow: hidden; position: relative; pointer-events: auto;">
    ...
    <form id="quoteForm" novalidate style="flex: 1; overflow: hidden; display: flex; flex-direction: column;">
            ...
            <div class="quote-tab-content" id="quote-tab-terms" style="padding: 1.5rem; display: none; overflow-y: auto; flex: 1;">
              ...
              <div class="form-group" style="margin-bottom: 1.25rem;">
                ...
                <div style="margin-top: 0.5rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
                  <button type="button" class="template-chip" data-template="standard">📄 Standard Terms</button>
                  <button type="button" class="template-chip" data-template="warranty">🛡️ Warranty</button>
                  <button type="button" class="template-chip" data-template="refund">💰 Refund Policy</button>
                </div>
              </div>
...
// Wire template chips without inline onclick
document.addEventListener('DOMContentLoaded', function () {
  ...
  Object.keys(sortableColumns).forEach(column => {
    ...
    if (header) {
      if (enable) {
        ...
        header.dataset.action = 'sort-column';
        ...
        delete header.dataset.sortColumn;
  ...
  allQuotations.forEach(quote => {
    ...
    row.innerHTML = `
      <td class="checkbox-column" style="display: ${checkboxesVisible ? '' : 'none'};">
        <input type="checkbox" class="quote-checkbox" value="${quote.id}" style="cursor: pointer;">
      </td>
      ...
      <td>
        <div class="flex gap-1">
          <button class="btn btn-ghost btn-sm" data-action="view-quote" data-quote-id="${quote.id}" title="View Details">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M1 12C1 12 5 4 12 4C19 4 23 12 23 12C23 12 19 20 12 20C5 20 1 12 1 12Z" stroke="currentColor" stroke-width="2"/>
              <circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="2"/>
            </svg>
          ...
          <button class="btn btn-ghost btn-sm text-success" data-action="approve-quote" data-quote-id="${quote.id}" title="Approve">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M5 13L9 17L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
          </button>` : ''}
          ${quote.status === 'approved' ? `
          <button class="btn btn-ghost btn-sm text-primary" data-action="convert-to-order" data-quote-id="${quote.id}" title="Convert to Order">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" stroke="currentColor" stroke-width="2"/>
            </svg>
          </button>` : ''}
          <button class="btn btn-ghost btn-sm" data-action="show-quote-actions" data-quote-id="${quote.id}" data-status="${quote.status || 'pending'}" title="More Actions">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="5" r="1" fill="currentColor"/>
              <circle cx="12" cy="12" r="1" fill="currentColor"/>
              <circle cx="12" cy="19" r="1" fill="currentColor"/>
...
function showQuoteActions(id, status, triggerEvent) {
  const e = triggerEvent || window.event;
  if (e) {
    e.stopPropagation();
  }
...
function bulkActionsMenu(triggerEvent) {
  const e = triggerEvent || window.event;
  if (e) {
    e.stopPropagation();
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  body.addEventListener('click', function (event) {
    ...
    if (!action || action === 'toggle-select-all' || action === 'change-items-per-page') {
      return;
    }
    ...
    switch (action) {
      ...
      case 'bulk-actions-menu':
        bulkActionsMenu(event);
        break;

      case 'sort-column':
        if (trigger.dataset.sortColumn) {
          sortTable(trigger.dataset.sortColumn);
        }
        break;

      case 'change-page':
        changePage(parseInt(trigger.dataset.direction || '0', 10));
        break;

      case 'go-to-page':
        goToPage(parseInt(trigger.dataset.page || '1', 10));
        break;

      case 'view-quote':
        viewQuote(trigger.dataset.quoteId);
        break;

      case 'approve-quote':
        approveQuote(trigger.dataset.quoteId);
        break;

      case 'convert-to-order':
        convertToOrder(trigger.dataset.quoteId);
        break;

      case 'show-quote-actions':
        showQuoteActions(trigger.dataset.quoteId, trigger.dataset.status, event);
        break;
      ...
      case 'quote-dropdown':
        handleQuoteAction(trigger.dataset.quoteAction);
        break;

      case 'bulk-dropdown':
        handleBulkAction(trigger.dataset.bulkAction);
        break;

      case 'attempt-close-quote-modal':
        if (event.target === trigger) {
          attemptCloseQuoteModal();
        }
        break;

      case 'cancel-close-quote':
        cancelCloseQuoteModal();
        break;

      case 'confirm-close-quote':
        confirmCloseQuoteModal();
        break;
    }
  });

  body.addEventListener('change', function (event) {
    if (event.target.matches('.quote-checkbox')) {
      updateBulkActions();
    }
