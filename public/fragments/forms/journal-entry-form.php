...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (isNewEntry) {
    ...
    clearStoredFiles();
    debugLog('🆕 New Entry - Cleared all stored data');
  }
  
  // Check if we're in edit mode - populate from PHP entry data
  if (isEditMode) {
    debugLog('✏️ Edit Mode - Loading entry data');
    <?php if ($editMode && $editEntry): ?>
    ...
    if (entryAttachments && entryAttachments.length > 0) {
      ...
      renderFileList();
      debugLog('✅ Loaded ' + entryAttachments.length + ' attachments');
    }
    ...
    debugLog('✅ Loaded ' + entryLines.length + ' lines and ' + entryTags.length + ' tags for editing');
    <?php endif; ?>
  }
  // Check if we're in copy mode - populate from PHP source data
  else if (isCopyMode) {
    debugLog('📋 Copy Mode - Loading source entry data');
    ...
    debugLog('✅ Copied ' + sourceLines.length + ' lines and ' + sourceTags.length + ' tags');
    <?php endif; ?>
  } else {
